...
===========================
接下来的两行代码显示出如何检查噢你用户是否有做某件事的权限。正如你所见，从开发者的角度来说，这些API都十分的简单。但是，在这下面实际上是一个复杂的接入系统。这会给管理员很大的伸缩性已控制什么人可以做什么。

Line 3: 获得上下文
--------------------
    $context = context_system::instance();                        // 3

在Moodles中，同一个人可能在不同的地方拥有不同的权限。比如说一个用户可能在某个课程上做一名老师，也可能是另一门课程的一位学生。这些地方被称作为上下文。上下文在Moodle中构筑了一个特别像文件系统中目录结构的多层结构。
...
![contexts.png](http://www.aosobook.org/images/moodle/contexts.png)  
图13.3：上下文
